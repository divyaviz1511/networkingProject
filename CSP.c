...
int main(int argc, char * argv[]) 
{
	...
	//fflush(out_file);
	output_file = fopen("CSP_OUTPUT.txt", "w");
			...
			if(FD_ISSET(sockfd, &rset))
			{
				...
				else
				{
					...
					if((strcmp(data,"Request") == 0))
					{
						...
						//printf("\nBefore Mapping Client address %d and descriptors %d", client[i],cliaddress[i].descriptor);
						...
						//printf("\nAfter Mapping Client address %d and descriptors %d", client[i],cliaddress[i].descriptor);
						//write log to file
			...
			for(j=0; j < asize; j++)
			{
				if(data_frame[j].sadd > 0) //checking if frame present.
				{
					...
					for(i = 0; i < maxi+1; i++) /*iterating through cliaddress to get the descriptor details*/
					{
						...
						{
							//printf("\nDescriptor value: %d", cliaddress[i].descriptor);
							//printf("\nSource : %d", data_frame[j].sadd);
							//printf("\nDest: %d", data_frame[j].dadd);
							//printf("\nSequence No: %d", data_frame[j].seqno);
